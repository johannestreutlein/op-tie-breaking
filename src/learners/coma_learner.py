...
from modules.critics.factored_coma import FactoredCOMACritic
...
class COMALearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        raw_q_vals, q_vals, critic_train_stats = self._train_critic_qvalues(batch, rewards, terminated, actions, critic_mask, bs)
        ...
        if self.test_v_baseline:
            v_vals, critic_train_stats = self._train_critic_vvalues(batch, rewards, terminated, critic_mask)
            baseline = v_vals.reshape(-1)
        elif self.args.test_vm_baseline:
            assert self.args.critic_fact == "vdn"
            baseline = (mac_out * raw_q_vals).sum(3).sum(2).unsqueeze(2).repeat(1, 1, self.n_agents).reshape(-1)
        elif self.args.test_rq_baseline:
            assert self.args.critic_fact == "vdn"
            products = raw_q_vals * mac_out
            products = products.view(-1, self.n_agents * self.n_actions)
            idx = th.multinomial(th.ones(products.shape, device=batch.device), self.n_actions)
            final = th.gather(products, dim = 1, index=idx)
            baseline = final.view(bs, -1, self.n_actions).sum(2).unsqueeze(2).repeat(1, 1, self.n_agents).reshape(-1)
        elif self.args.test_maxq_baseline:
            assert self.args.critic_fact == "vdn"
            max_q = (th.max(raw_q_vals, dim = 3)[0]).sum(2, keepdim=True).repeat(1, 1, self.n_agents)
    ...
    def _train_critic_vvalues(self, batch, rewards, terminated, mask):
        ...
        targets = build_td_lambda_targets(rewards, terminated, mask, target_v_vals, self.n_agents, self.args.gamma,
                                          self.args.td_lambda)
        ...
        for t in reversed(range(rewards.size(1))):
            ...
            loss.backward()
            grad_norm = th.nn.utils.clip_grad_norm_(self.baseline_critic_params, self.args.grad_norm_clip)
            ...
            running_log["q_taken_mean"].append((v_t * mask_t).sum().item() / mask_elems)
    ...
    def _train_critic_qvalues(self, batch, rewards, terminated, actions, mask, bs):
        # Optimise critic
